<template>
 <div>
    ...
    <transition enter-active-class="animated fadeIn" mode="out-in" appear >
      <data-table tabla="correo" :selcol="columnas" @viewDetail="fila=$event;vista=1" v-show="vista===0" @agregar="vista=3" ref="datatable"></data-table>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    this.setNav([
      ...
      { text: "Configurar Cuenta", name: "" }
    ]);
  },
  computed: {
    rutaEliminar() {
      return `correo/${this.registro.email}`;
    }
  },
  methods: {
      ...
      switch (this.vista) {
        case 1:
          this.setCurrent({
            icon: "fa fa-list-alt",
            title: "Cuentas",
            subtitle: `Datos del registro ${this.fila.email}`
          });
          break;
        case 2:
          this.setCurrent({
            icon: "fa fa-edit",
            title: "Cuentas",
            subtitle: `Editando el registro de ${this.fila.email}`
          });
          break;
        case 3:
          this.setCurrent({
        ...
        case 4:
          this.setCurrent({
            icon: "fa fa-trash",
            title: "Cuentas",
            subtitle: `Eliminando el registro de ${this.fila.email}`
          });
          break;
        default:
          this.setCurrent({
